...
function handle(basket) {
    ...
    if (prefs.isMFRA) {
        selectedCreditCardUuid = httpParameterMap.braintreeCreditCardList.stringValue;
    } else {
        selectedCreditCardUuid = httpParameterMap.dwfrm_billing_paymentMethods_creditCardList.stringValue;
    }
    ...
    } else if (prefs.isMFRA) {
        creditCardForm = session.forms.billing.creditCardFields;
        creditCard = {
    ...
    Transaction.wrap(function () {
        try {
            ...
            var paymentInstrument = basket.createPaymentInstrument(PaymentInstrument.METHOD_CREDIT_CARD, BraintreeHelper.getNonGiftCertificateAmount(basket));
...
function authorize(orderNo, paymentInstr) {
    ...
    if (paymentInstrument && paymentInstrument.getPaymentTransaction().getAmount().getValue() > 0) {
        ...
        try {
            ...
            saleTransactionResponseData = braintreeApiCalls.call(saleTransactionRequestData);
        ...
        if (paymentInstrument.custom.braintreeCreditCardMakeDefault) {
            braintreeApiCalls.makeDefaultCreditCard(saleTransactionResponseData.transaction.creditCard.token);
            Transaction.wrap(function () {
